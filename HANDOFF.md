...
## The production walk, `a51ffb2`

**No production gap.** The deployed stylesheet is **byte-identical** to the local build of the same
SHA (`md5 0c1f761483f1ec14f940c0e49f680b75`), and all five chip tones are live in it — including
`bg-red-500/10`, which has no adopter yet and is present because Tailwind sees it in the
`CHIP_TONES` literal, so the first surface to reach for `danger` will get a styled chip rather than
an unstyled one.

**Zero assistant trace**, scanned over **1.08 MB** — the served HTML, the stylesheet and all 11
JS chunks — plus a separate scan of `/compare/`, `/methods/`, `/privacy/`, `/stitch/` and
`/validation/`. No hits for any vendor or model name, `co-authored-by`, or `generated by/with`.

**The unwalked journey was `/stitch`, chosen because that is where the neutral-chip defect had been
living.** Real journey, not a deep link: two files analysed, both ticked in the logbook, compared,
then stitched. "Burn located `from · measured`" now renders as a visible tile in **both** themes —
it had been a bare outline in dark mode since the chip was written, and nothing had ever caught it.
No console errors on either pass.

**Playwright cannot reach production through the agent proxy** (`ERR_CONNECTION_RESET`, with and
without `proxy.server` set), while `curl` goes through fine. So the production check is curl-based
— HTML, CSS and every chunk fetched and scanned — and the browser walk runs against the local
`out/` of the shipped SHA, which the byte-identical stylesheet proves is the same build. Worth
knowing before spending an increment on it: it is an environment limit, not a site problem.

## The corpus sweep, and it is empty

**`lib/parsers/corpus.test.ts`: 142 assertions over the 61-fixture manifest, 0 findings.** Stated
because an empty sweep is a result: this run's shipped work was a UI primitive and a design-system
ratchet, neither of which can move a number, and the corpus confirms none did.

**A second sweep, run for D9 and also empty, and this one is load-bearing:** zero `.ork`, zero
`.rkt`, zero `.rse`, zero `.ric` anywhere under `lib/parsers/__corpus__`, and zero manifest hits for
`simulat|predict`. That empty result is exactly why D9's first slice is sourcing a file rather than
writing a parser.

## The two track questions, answered

**D-track — what can a flyer DO that they could not before?** *Nothing new from this increment, and
that is the honest answer.* D8's remaining slice is measured and blocked on a discriminator the
corpus cannot supply, so the D-track's work this increment was **decomposing D9** — planning, not
capability. Earlier increments in the run did ship D-track capability (a replayed high-rate block is
now detected and stated). What stopped D9 going further is a **missing fixture**, not a decision:
...
4. **D9 is DECOMPOSED and its first slice is not code — it is a FILE.** With D8 slice 4 blocked, the
   ...
   **Two measurements in that decomposition will save an increment each, and the second one reverses
   ...
   scalars as STATED FIGURES** — it is over-50 per-timestep plot variables, user-selected, with no
   stable schema and no fixed separator. Five of the ten (`maxaltitude`, `maxvelocity`,
   `maxacceleration`, `maxmach`, `flighttime`) are *derivable* from those columns and a sixth,
   `timetoapogee`, appears in the optional events comment block — so "the CSV carries none of them"
   is wrong, and this repo's own `COMPETITION.md` row 22 already records the events block. The real
   disqualifier is that deriving a maximum from a user-chosen, user-decimated series is not reading
   a stated figure. All ten ARE stated in `<flightdata>` inside a **`.ork`**, whose element is named
   on OpenRocket's own file-specification page. A first pass recommended the CSV precisely because it believed the `.ork`
   was a clean-room hazard, citing two phrases ("not documented", "reference implementation") that
   **are not on that page** — its one XSD sentence is about `.orc`, a different format. Both halves
   were re-fetched and checked directly rather than taken on the finding's word.

   **But do not read that as "the hazard was imaginary", which a first draft of this entry did.**
   The page's actual sentence is *"Not every parameter in the XML file is explained here. Please
   refer to the `core/src/main/java/info/openrocket/core/file/openrocket` package in the source
   code."* — it defers to GPL-3 source for anything it does not explain. The ten attributes ARE
   named on the page, so slice 2's summary parser can be written from it; the `<databranch>` series
   and the `status` vocabulary are where the deferral bites, and that is a boundary to respect. It
   is a **weaker** footing than `lib/parsers/xlsx.ts`, which cites two complete formal specs. The
   page also states **no units** — SI is inferable from the example's own arithmetic and from the
   dev guide, not from the spec page, so slice 2 proves SI against a real file rather than assuming
   it.

   And the repo is already shaped for it: `ReportedValue.source` is the single-member union
   `'device'` (`lib/flight/types.ts:91`), `compareReported` holds the agree/differ logic once, and
   `lib/parsers/deviceSummary.ts` is the precedent for a registered parser that is not a flight. What
   must NOT be bent to fit is `lib/compare.ts` — its `CompareInput` demands a full `FlightAnalysis`,
   so feeding a prediction in would fabricate detected events and metrics for a curve that has none.
