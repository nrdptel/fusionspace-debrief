...
test('a GPS log shows the recovery (ground track) view with walkback numbers', async ({ page }) => {
  ...
  // corpus GPS flight that a second instrument also recorded puts this read 5% above the
  // measured speed and 9% above the tracker's own summary, always high; every other derived
  // peak the corpus can check runs high too, by 23%, 30% and 110%. The barometric
  // explanation would name the wrong sensor and the wrong failure, so the GPS sentence is
  // its own.
