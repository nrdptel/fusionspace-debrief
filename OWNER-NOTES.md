...
- **2026-08-18 — ZERO-TRACE BREACH I could not undo: one comment on `#208` carries a harness
  attribution footer. Please delete the comment; nothing else is affected.** The comment is
  <https://github.com/nrdptel/fusionspace-debrief/pull/208#issuecomment-5325577441>. It corrects a
  false measurement in `#208`'s merged body (see `ROADMAP.md` D12 slice 1), and the correction
  itself is worth keeping — but the GitHub MCP server appends *"Generated by Claude Code"* to every
  comment it posts, **server-side, after the body is submitted**, and the session's direct GitHub
  API access is blocked in both directions (`403 GitHub access is not enabled for this session`), so
  there is no edit or delete path from here.

  **What was tried**: composing the body without a footer (the server appended one anyway); a REST
  `PATCH` on the comment (403); a REST `GET` to recover `#208`'s exact body so the correction could
  be folded into the body instead (403 — reads are blocked too, so the body could not be edited
  without retyping it and risking a transcription error in a merged artifact).

  **Why this is filed rather than left**: pull-request BODIES are safe — the update path strips the
  footer, verified on `#209`. It is only COMMENTS that cannot be cleaned. **A session should
  therefore prefer editing a pull request's body over posting a comment**, and post a comment only
  when the alternative is leaving a false claim standing, which is the trade made here.

  The correction survives your deleting the comment: it is in `ROADMAP.md` D12 slice 1, in
  `BACKLOG.md` as a withdrawn entry, and in `#209`'s body, all three footer-free.
