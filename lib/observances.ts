...
//
// It switches with the calendar: evaluated from `new Date()` when the static
// HTML is generated. The site is rebuilt on every deploy and on a monthly
// schedule (see .github/workflows/deploy-cloudflare.yml), so an observance
// appears and disappears within that window of the month rolling over — no
// per-request work and no manual toggling.
...
const MENS_MH = 'linear-gradient(to right, #34d399, #059669)'; // emerald ribbon
