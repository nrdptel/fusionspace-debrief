...
describe('a synthetic flight says so wherever it can go', () => {
  ...
  const SINKS: {
    ...
    { name: 'cross-check table copy (DeviceSummary)', state: 'labelled', why: 'ADDED 2026-08-17 by the pre-push review of D10\'s design sample, and it is a sink the SAMPLE created rather than one the audit had missed all along. `DeviceSummary` renders only where `flight.reported` is non-empty — a file carrying a logger\'s own summary, or a design paired with it — and every made-up sample before the design pair did neither, so no synthetic flight could ever reach it. It was consequently the one table-bearing child of `FlightReport` never passed `synthetic`, and `DataTable`\'s copy button emits exactly `columns.map(c => c.header)`, so "Copy this cross-check" wrote an invented prediction beside an invented flight with nothing saying either was made up. The claim rides on the column HEADERS through `syntheticHeader`, on that function\'s own stated argument: a column is the unit that travels, and a caption row is a cell a sort moves away from the rows it was about. **The transferable lesson is about `todo: 0` rather than about this table**: a new CAPABILITY can create a sink that no previous audit could have enumerated, because the state it lives in did not exist. `todo: 0` means nothing KNOWN is open, which this milestone\'s notes already say and which this is the fourth demonstration of.', check: { file: 'e2e/analyze.spec.ts', contains: 'the cross-check table a cert document quotes carries it too' } },
  ...
  it('accounts for every sink the audit found — none silently forgotten', () => {
    ...
    // is a total.
    expect(SINKS.length, 'a new exporter must be added to SINKS').toBe(30);
