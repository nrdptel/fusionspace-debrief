...
export type Sample = {
   ...
   *
   * `'design'` means the set is a flight PLUS an OpenRocket design of the same rocket, which is
   * droppable on the analyze page and — like `stages`, and for a different reason — must not be
   * what `/compare` offers. That surface compares RECORDINGS, and a design is not a second
   * recording of anything: opening this pair there would leave one flight in a comparison built
   * for two, demonstrating the surface failing. The `stages` exclusion below was written after
   * exactly this shape of mistake; this is the same trap one file over, closed before it fires
   * rather than after.
   *
   ...
   * on its own terms.
   */
  kind?: 'flight' | 'mapping' | 'stages' | 'design';
...
export const SAMPLES: Sample[] = [
  ...
  {
    id: 'coarse-gps',
    label: 'A tracker whose fix kept changing',
    shows:
      'A GPS-only recording where the receiver had a good fix, then a poor one, then none at all — and Debrief says which was which rather than drawing them all the same. This one is MADE UP, and says so on every surface it reaches.',
    // No figure, for the reason the other generated offers record — and one specific to this file.
    // The apogee a reader would quote off it is read from the RECEIVER'S OWN altitude, and this
    // sample exists to show that some of those heights are withheld because the fix behind them was
    // two-dimensional. Putting a height on the button would advertise the number the file's whole
    // point is that Debrief will not always give you.
    // **No number anywhere in this line, and the check caught the first draft stating one.** It read
    // "a steady 6 m/s westerly" — a parameter of the generator, not a reading of the file, and
    // `lib/samples.test.ts` refuses a figure that is neither Debrief's own read of the sample's
    // files nor written in them. It is right to: an offer is prose beside a file, and a generator
    // constant is exactly the kind of number that goes on being quoted after the curve moves.
    source:
      'Generated by lib/synthetic.ts — a piecewise curve, not a simulation and not a recording. The same flight the front door’s made-up samples fly, sampled once a second like a tracker and drifting downwind the whole way; the satellite count is a curve chosen to reach each fix grade, not a model of a receiver.',
    files: ['sample-gps-tracker.csv'],
    synthetic: true,
  },
  {
    id: 'staged-pair',
  ...
  {
    id: 'design-overlay',
    label: 'A design, and the flight it flew',
    shows:
      'An OpenRocket design set against a recording of the same rocket — what was predicted, what was flown, and the gap. BOTH are MADE UP, and both say so on every surface they reach.',
    // No figure, for the reason the other generated offers record — and one more that is specific
    // to this pair. The number a reader would quote off a predicted-versus-flown sample is the
    // MISS, and a miss between two invented curves is the one figure here that could be mistaken
    // for a finding about a simulator. The report is where the comparison belongs.
    source:
      'Generated by lib/synthetic.ts and lib/syntheticDesign.ts — a piecewise curve and a hand-built .ork, neither a simulation nor a recording. The design is not OpenRocket’s work and no rocket was ever designed; the log’s column names are an Eggtimer Classic’s so Debrief reads it without being told.',
    files: ['sample-design-flight.csv', 'sample-design.ork'],
    kind: 'design',
    synthetic: true,
  },
];
...
export const MULTI_SAMPLE: Sample | undefined = SAMPLES.find(
  (s) => s.files.length > 1 && s.kind !== 'stages' && s.kind !== 'design',
);
